!(function () {
  ...
  const h = 5,
    ...
    w = 6048e5,
    _ = 26784e5;
  var x = { SECOND: 1e3, MINUTE: v, HOUR: y, DAY: b, WEEK: w, MONTH: _ },
    S = {
      apiUrl: a.get(
  ...
  ******************************************************************************/
  ...
  const { $influx: mb } = app,
    vb = {
      ...
      isAcknowledged: function (e) {
        return -1 !== mb.model.state.acknowledged[e];
      },
      userId: function () {
        return mb.model.state.authStatus.userId;
      },
      username: function () {
        return mb.model.state.authStatus.username;
      },
      allUsernames: function () {
        const e = mb.model.state,
          t = [],
    ...
    { $influx: yb } = app;
  var bb = yb.action("state.replace-state", (e, t) => t);
  const { $influx: wb } = app;
  var _b = wb.action("state.acknowledge", (e, t) => {
    ...
    kb = () => ({
      ...
      quickReplies: { shown: !1, content: Pb(), total: 7 },
      settings: { laterAutoRetry: 12 * j.time.HOUR },
      tagAssist: {
        shown: !1,
        query: "",
  ...
  var Db = {
      ...
      replaceState: bb,
      getWhatsNewItems: xb,
      getTemplateUserState: Sb,
      getTemplateSharedState: kb,
      actions: { acknowledge: _b },
    },
    Ib = {},
    Eb = {},
    Tb = {};
  const { $influx: Cb, $chromeBus: Fb } = app;
  let Ab = null,
    Ob = !1;
  Tb.controller = {
    init: async function () {
    ...
    refreshUser: async function () {
      ...
      const r = n.result || null,
        o = r && Cb.model.state.authStatus.userId === r.userId;
      return (
        Cb.transaction((e) => {
          ...
          if (e.authStatus.userId) {
            ...
            const t = Db.getTemplateUserState();
        ...
        o ||
          ...
          r
            ? Ab
              ...
              : (Ib.controller.cleanUpState(), Fb.send("igView.refresh"))
    ...
    _updateUserOnPopupStart: function () {
      ...
      Fb.on("popup.start", async () => {
        ...
        const r = await j.callAsync(chrome.cookies.get, {
          ...
          o =
            null === (t = Cb.model.state.authStatus) ||
            void 0 === t ||
            null === (n = t.cookies) ||
            void 0 === n
              ? void 0
    ...
    _setAuthStatusIfAbsent: async function () {
      if ("username" in Cb.model.state.authStatus) return;
      db.ignoreCache();
    ...
    _watchAndSaveFbCookies: function () {
      let e;
      Eb.controller.onRequest(({ details: n, isResponse: r, checkHost: o }) => {
        if (r && o("facebook.com")) {
    ...
    _login: async function (e) {
      var t;
      const n =
        null === (t = Cb.model.state.userStates[e]) || void 0 === t
          ? void 0
          : t.authStatus;
      if (n) {
  ...
  const { $chromeBus: Gb, $files: Wb } = app;
  gb.cleanupController = {
    init: function () {
      (this._lastIdbCleanupAt = 0),
        Gb.on("popup.start", () => this._cleanupIdb());
    },
    _cleanupIdb: async function () {
      const e = Date.now();
      if (e - this._lastIdbCleanupAt < 5 * j.time.MINUTE) return;
      this._lastIdbCleanupAt = e;
      const t = await Wb.controller.getFileIds("later"),
        n = gb.proxy.getAllPosts();
      for (const e of t) {
        !!n.find((t) =>
          t.mediaList.some(
            (t) => t.fileId === e || t.coverId === e || t.previewId === e
          )
        ) || (await Wb.controller.remove(e));
      }
    },
  };
  const { $influx: Jb } = app;
  gb.controller = {
    init: function () {
      this._cleanupState(), gb.scheduler.init(), gb.cleanupController.init();
    },
  ...
  gb.scheduler = {
    ...
    _schedule: function () {
      ...
      const n = gb.proxy.getPostOwnerState(e, t.id),
      ...
      if (
        ...
        return (
          ...
          void gb.proxy.updatePost(t.id, (e) => {
            (e.status = "failed"),
      ...
      (async () => {
        this._log("publish", o),
          (this._publishing = !0),
          Kb.controller.online
            ? await this._publishSafe(t)
            : await Kb.controller.waitForOnline(),
          (this._publishing = !1),
          this._schedule();
      })();
    },
    _publishSafe: async function (e) {
      const t = gb.proxy.getPostPublishTypeForAnalytics(e);
      Tb.controller.toggleSessionWatcher(!1);
      try {
        await this._publish(e),
          Qb.controller.sendEvent("user", "later:publish-success", t);
      } catch (r) {
        var n;
        console.error(r);
        const o =
        ...
        Qb.controller.sendEvent("user", "later:publish-error", t),
          Qb.controller.sendEvent("user", "later:publish-error-message", o),
          gb.proxy.updatePost(e.id, (e) => {
            (e.status = "failed"),
              (e.error = {
    ...
    _publish: async function (e) {
      ...
      if (Kb.controller.offline)
        return void gb.proxy.updatePost(e.id, (e) => (e.status = "scheduled"));
      const a = (e.caption || "").slice(0, gb.config.maxCaptionLength),
        s = new hb.Publisher(i);
      if ("post" === e.type || "reel" === e.type) {
        const t = Yb.validateCaption(a);
        if (t)
          return void gb.proxy.updatePost(e.id, (e) => {
            (e.status = "failed"),
              (e.error = {
      ...
      for (const t of e.mediaList) {
        const n = gb.proxy.getMediaError(e, t);
        if (n)
          return void gb.proxy.updatePost(e.id, (e) => {
            (e.status = "failed"),
              (e.error = {
      ...
      if ("post" === e.type)
        if (e.mediaList.length > 1) {
          ...
          const t = await Zb.controller.read(e.mediaList[0].fileId),
            n = await Zb.controller.read(e.mediaList[0].coverId);
        ...
        for (const t of e.mediaList) {
          const n = t === e.mediaList[0],
            r = await Zb.controller.read(t.fileId),
            o = await Zb.controller.read(t.coverId);
          await s.postReel(r, o, {
            ...
            await j.sleep(gb.config.minTimeBetweenPublishing);
        }
      ...
      e.instant ||
        (Xb.transaction((e) => (e.billing.trial.later += 1)),
        true &&
          Qb.controller.sendEvent("user", "pro-paid-usage:later")),
        gb.proxy.updatePost(e.id, (e) => {
    ...
    _prepareCarouselFiles: async function (e) {
      ...
      for (const n of e.mediaList) {
        const e = await Zb.controller.read(n.fileId),
          r = n.coverId && (await Zb.controller.read(n.coverId));
      ...
      {
        const e = gb.config.minImageRatio,
          t = gb.config.maxImageRatio;
      ...
      return t
        .filter((e) => {
          const t = gb.config.maxCarouselVideoDurationSec;
  ...
  gb.proxy = {
    ...
    getAllPosts: function (e = ew.model.state) {
    ...
    getMediaError: function (e, t) {
      return "post" === e.type &&
        e.mediaList.length > 1 &&
        t.isVideo &&
        t.duration > gb.config.maxCarouselVideoDurationSec
        ? {
            message:
        ...
        : "story" === e.type &&
          t.isVideo &&
          t.duration > gb.config.maxStoryDurationSec
        ? {
            message:
  ...
  var ow = { controller: tw };
  const { $chromeBus: iw } = app;
  const aw = "__iframeBus.name",
    sw = "__iframeBus.args",
    lw = "__iframeBus.callbackId",
    uw = "undefined" != typeof parent && parent !== window;
  function cw(e, t) {
    const n = hw(e),
  ...
  async function pw(e, ...t) {
    ...
    if (
      (uw
        ? parent.postMessage({ [aw]: i, [sw]: t, [lw]: a }, "*")
        : (function (e, t = document) {
            ...
            e.contentWindow.postMessage({ [aw]: i, [sw]: t, [lw]: a }, "*");
          }),
      !o)
    )
      return new Promise((t) => {
        const r = (o) => {
          n && n(o), fw(`${e}:response-${a}`, r), t(o);
        };
        cw(`${e}:response-${a}`, r);
      });
  }
  ...
  var gw = {
    init: function () {
      iw.on("iframe-bus", (e, ...t) => pw(e, ...t)),
        cw("chrome-bus", (e, ...t) => iw.send(e, ...t));
    },
    on: cw,
    once: dw,
    off: fw,
  ...
  const { $influx: mw, $chromeBus: vw } = app;
  Qb.controller = {
    init: function () {
      return (
        d.is.popup
    ...
    _initIframeMessage: function () {
      gw.on("ga.send-event", (...e) => {
        this.sendEvent(...e);
      });
    },
    _initChromeMessage: function () {
      vw.on("ga.send-event", (...e) => {
        this.sendEvent(...e);
      });
    },
    _sendBgEvents: async function () {
      const e = (await ow.controller.get("ga.bgEvents")) || [];
      await ow.controller.set("ga.bgEvents", []);
    ...
    sendInstall: function () {
      if (!mw.model.state.installedEventSent) {
    ...
    _ga: async function (...e) {
      if (d.is.background) {
        const t = (await ow.controller.get("ga.bgEvents")) || [];
        t.push(e), await ow.controller.set("ga.bgEvents", t);
      } else {
  ...
  var yw = t({});
  const { $influx: bw, $chromeBus: ww } = app;
  function _w(e) {
    bw.transaction((t) => {
      (t.cleanupId = e),
      ...
      r.length > 0 &&
        ...
        (t.tagAssist.query = r
          ...
          .map((e) => `#${e}`)
          .join(" "))),
        Object.assign(t.coverAssist, yw.defaultState),
  ...
  var xw = {
      getConfig: () => {
        ...
        return e && e.version >= xw.config.version ? e : xw.config;
      },
    },
    Sw = {};
  const { $chromeBus: Pw } = app;
  let kw, Dw;
  function Iw() {
  ...
  async function Ew() {
    ...
    chrome.extension.getViews({ type: "tab" }).length > 0
      ? Pw.send("fusion.new-version-available")
      : Tw();
  }
  function Tw() {
    Dw && (j.ls.set("fusion.config", Dw), location.reload());
  }
  function Cw(e, t) {
    for (const n in t)
      j.isObject(e[n]) && j.isObject(t[n]) ? Cw(e[n], t[n]) : (e[n] = t[n]);
    return e;
  }
  (xw.controller = {
    init: function () {
      Pw.on("fusion.check-new-version", Ew),
        Pw.on("fusion.popup-tab-id", (e) => {
          kw = e;
        }),
        Sw.controller.onReset(Iw),
        chrome.tabs.onRemoved.addListener((e) => {
          e === kw && Dw && Tw();
        }),
        Pw.on("fusion.update-now-click", () => {
          j.ls.set("fusion.reload-popup-on-background-start", !0), Tw();
        }),
        j.ls.get("fusion.reload-popup-on-background-start") &&
          (j.ls.remove("fusion.reload-popup-on-background-start"),
          Pw.send("fusion.reload-popup")),
        chrome.alarms.onAlarm.addListener(async (e) => {
          "fusion.refresh-config" === e.name && Ew();
        }),
  ...
  Fw.controller = {
    ...
    _watchForPopupTab: function () {
      Aw.on("wri.popup-tab-created", async (e) => {
    ...
    _updateUserWhenOnlineStatusChanges: function () {
      this._onOnline(() => {
        log("[$chromeStarter] going online"),
          null === Ow.model.state.authStatus.userId &&
            Tb.controller.updateUser();
      }),
        this._onOffline(() => {
  ...
  const { $insights: Mw } = app;
  Mw.credibilityToGrade = (e) => {
    const t = Rw.find((t) => t.condition(e));
  ...
  Bw.controller = {
    ...
    _getCredibilityGrade: function (e) {
      const t = jw.model.state.authStatus.userId,
        n = this._getCredibility(e, t);
      return Bw.credibilityToGrade(n);
    },
  ...
  Vw.controller = {
    save: async function (e, t = "") {
      const n = Rg.generate(),
        r = t ? `${t}.${n}` : n;
      return await ow.controller.set(`files.${r}`, e), r;
    },
    read: async function (e) {
      return (await ow.controller.get(`files.${e}`)) || null;
    },
    remove: async function (e) {
      await ow.controller.delete(`files.${e}`);
    },
    getFileIds: async function (e = "") {
      const t = await ow.controller.getAllKeys(),
        n = e ? `files.${e}.` : "files.";
      return t
  ...
  const { $influx: zw } = app;
  async function Hw(e = 0) {
    const t = zw.model.state,
      n = t.authStatus.userId;
    if (!n) return;
    const r = Date.now(),
      o = $w.proxy.getAccountStats(),
      i = o && r - o.lastScanOn;
    ...
    const f = $w.utils.sortByFrequency(
      l.map((e) => e.caption || "").map((e) => $w.utils.extractTags(e, !0))
    );
    zw.transaction((e) => {
  ...
  ($w.accountStatsController = {
    ...
    ($w.controller = {
      init: function () {
        $w.accountStatsController.init();
      },
    });
  const { $influx: qw } = app;
  $w.proxy = {
    getAccountStats: function () {
      const e = qw.model.state,
  ...
  var Ww = {
    ...
    Jw = {};
  const { $influx: Qw, $chromeBus: Yw } = app;
  var Kw = {
    init: function () {
      Yw.on("overseer.send-report", e_);
    },
    sendReport: e_,
  };
  const Xw = [
    ...
    Zw = new Xg.Sender({ urlPrefix: d.options.apiUrl });
  async function e_({ key: e, filters: t, data: n } = {}) {
    if (
      ((e = e || "system"),
      (n = n || {}),
      (t = t || {}).username || (t.username = Db.proxy.username() || "unknown"),
    ...
    d.is.development &&
      Yw.send("popup.log", "%coverseer report [background]", "color: #c818dc", {
        key: e,
        filters: t,
        data: JSON.parse(n),
      }),
      Zw.send("/overseer", { body: r })
        .then((t) => {
  ...
  var t_ = { controller: Kw },
    n_ = {};
  const { $influx: r_, $chromeBus: o_, $insights: i_, $files: a_ } = app;
  n_.controller = {
    init: function () {
      (globalThis.$env = d),
        (globalThis.ig = pb),
        (globalThis.utils = P),
        (globalThis.$igApi = hb),
        (globalThis.$eventBus = Ww),
        (globalThis.$chromeBus = o_),
        (globalThis.$iframeBus = gw),
        ...
        (globalThis.$overseer = t_),
        (globalThis.$insights = i_),
        ...
        (d.is.development || d.is.beta) &&
          ((globalThis.model = r_.model),
          (globalThis.transaction = r_.transaction),
          (globalThis.$utils = j),
          (globalThis.$synch = Jw),
          (globalThis.$state = Db),
          (globalThis.$later = gb),
          (globalThis.$files = a_),
          (globalThis.$influx = r_),
          (globalThis.$chromeStarter = Kb),
          (globalThis.$webRequestInterceptor = Eb),
          this.defineCommit());
    },
    ...
    setState: function (e) {
      ...
      Db.replaceState.dispatch(t);
    },
    downgradeVersion: function () {
      const e = Xy.cloneDeep(r_.model.state);
      (e.version = e.version - 1), Db.replaceState.dispatch(e);
    },
    defineCommit: function () {
      Object.defineProperty(globalThis, "commit", {
        get: () => (this.setState(r_.model.state), null),
      });
    },
  };
  var s_ = t(function (e) {
  ...
  const { $influx: l_, $chromeBus: u_ } = app;
  let c_, d_, f_, p_, h_, g_, m_, v_, y_, b_, w_, __;
  function x_() {
    (b_ = "Me"),
      (g_ = "im"),
      (f_ = "me"),
      ...
      S_(),
      u_.on("popup.start", S_),
      u_.on("ig.checkLogin", P_);
  }
  function S_() {
    if (!l_.model.state.authStatus.isConnected) return;
    const e = window[c_ + d_ + f_][p_ + h_ + g_ + m_],
      t = e[v_ + y_ + b_ + w_ + __ + m_].bind(e);
    e[v_ + y_ + b_ + w_ + __ + m_] = (...e) => {
  ...
  const { $inspiration: k_, $chromeBus: D_, $files: I_ } = app;
  k_.controller = {
    init: function () {
      (this._lastIdbCleanupAt = 0),
        D_.on("popup.start", () => this._cleanupIdb()),
        x_();
    },
    _cleanupIdb: async function () {
      const e = Date.now();
      if (e - this._lastIdbCleanupAt < 5 * j.time.MINUTE) return;
      this._lastIdbCleanupAt = e;
      const t = await I_.controller.getFileIds("inspiration"),
        n = k_.proxy.getPosts();
      for (const e of t) {
        !!n.find((t) => t.fileId === e || t.previewId === e) ||
          (await I_.controller.remove(e));
      }
    },
  };
  const { $inspiration: E_, $influx: T_ } = app;
  E_.proxy = {
    getPosts: function () {
      return T_.model.state.inspiration.posts;
    },
    getPost: function (e) {
      return T_.model.state.inspiration.posts.find((t) => t.id === e);
    },
  ...
  const { $chromeBus: C_ } = app;
  var F_ = {
    init: function () {
      (O_ = -1),
        C_.on("core-web-request.popup-tab-id", (e) => {
          A_ = e;
        });
    },
    watch: function (e, t) {
  ...
  function R_(e) {
    return e.tabId === A_ || e.tabId === O_;
  }
  var U_ = { controller: F_ };
  Eb.controller = {
    init: async function () {
      (this._wwwClaim = null),
  ...
  const { $influx: B_ } = app,
    j_ = { ...Db.getTemplateUserState(), ...Db.getTemplateSharedState() },
    L_ = B_.action("synch.synch-state", (e, t) => t);
  Jw.controller = {
    isStorageMaster: !1,
    currentState: null,
    unsubscribe: null,
      ...
      return (
        chrome.runtime.onMessage.addListener((t, r, o) => {
          ...
          if ("synch-state" === t.name) {
            if (!B_.model.store) return;
            return this._onStateUpdatedByPeer(t.deltaState), !1;
          }
          return (
            "fetch-state" === t.name && (n ? n.push(o) : o(B_.model.state), !0)
          );
        }),
        Promise.resolve()
          ...
          .then(() => {
            const e = n;
            (n = null), e.forEach((e) => e(B_.model.state));
          })
          .then(() => {
            this._subscribeToInflux();
          })
    ...
    _fetchState: function () {
      return this.isStorageMaster
        ? Jw.storageController
            .init()
            .then((e) => e || j_)
            .then((e) => {
              (this.currentState = e), B_.model.init(e);
            })
        : new Promise((e) => {
            chrome.runtime.sendMessage(
              { name: "fetch-state", sender: this.id },
              (t) => {
                (this.currentState = t), B_.model.init(t), e();
              }
            );
          });
    },
    _subscribeToInflux: function (e = !1) {
      this.unsubscribe = B_.model.observe(
        (e) => e,
        (e) => {
    ...
    _onStateUpdatedByPeer: function (e) {
      this.unsubscribe && this.unsubscribe(),
        (this.currentState = { ...B_.model.state, ...e }),
        L_.dispatch(this.currentState),
        this._saveToStorage(this.currentState),
        this._subscribeToInflux(this.currentState !== B_.model.state);
    },
    _saveToStorage: function (e) {
      this.isStorageMaster &&
        ...
        (this.storeBatchingId = setTimeout(
          () => {
            (this.storeBatchingId = null), Jw.storageController.save(e);
          },
  ...
  Jw.storageController = {
    ...
    read: async function () {
      const e = Date.now(),
        t = await ow.controller.get("state");
      if (!t) return null;
    ...
    save: async function (e) {
      const t = Date.now(),
        n = this._uglify(e);
      await ow.controller.set("state", n);
      const r = Date.now() - t;
    ...
    _readStateFromDeprecatedStorage: async function () {
      ...
      if (
        ...
        return null;
      try {
        const e = V_.decompressFromUTF16(t);
        n = JSON.parse(e);
      } catch (e) {
        return console.error(e), null;
      }
  ...
  const { $chromeBus: $_ } = app;
  Sw.controller = {
    _emitter: j.createEmitter(),
    reset: function () {
      this._emitter(), $_.send("reset.reset");
    },
    onReset: function (e) {
      this._emitter(e), $_.on("reset.reset", e);
    },
  };
  var z_ = {};
  const { $influx: H_ } = app;
  ...
  var q_ = {};
  const { $influx: G_, $chromeBus: W_ } = app;
  function J_() {
    return {
      hasPro: true,
      freeReels: Math.max(0, 2 - G_.model.state.billing.trial.reels),
      maxFreeReels: 2,
    };
  }
  async function Q_(e) {
  ...
  async function Y_() {
    Qb.controller.sendEvent("user", "reels:submit", "desktop"),
      true
        ? Qb.controller.sendEvent("user", "pro-paid-usage:reels", "desktop")
        : G_.transaction((e) => {
  ...
  q_.controller = {
    init: function () {
      W_.on("desktop-reels.get-initial-data", J_),
        j.watchForIgCookie("desktop-reels.open-billing", Q_),
        j.watchForIgCookie("desktop-reels.submit-success", Y_),
        j.watchForIgCookie("desktop-reels.get-initial-data", K_);
    },
  };
  var X_ = {};
  X_.controller = {
    init: async function () {
      await (async function () {
        ...
        chrome.webRequest.onHeadersReceived.addListener(ex, { urls: Z_ }, [
          "blocking",
          "responseHeaders",
          "extraHeaders",
        ]);
  ...
  var lx = {},
    ux = {
      update: function (e) {
        return e.userStates && ow.controller.delete("tag-assist.tag-data"), e;
      },
    };
    ...
    qS = {
      update: function (e) {
        ...
        return (
          ...
          delete t.insights,
          (async function () {
            const e = await ow.controller.getAllKeys();
            for (const t of e)
              (t.startsWith("insights.") || t.startsWith("block:analytics:")) &&
                ow.controller.delete(t);
          })(),
          t
        );
      },
  ...
  var nP = {},
    rP = {
      update: function (e) {
        if (!e.userStates) return e;
        return { ...e, quickReplies: { shown: !1, content: Pb(), total: 7 } };
      },
    };
      ...
      update: async function (e) {
        ...
        try {
          const e = await new Promise((e, t) => {
            ...
            n = V_.decompressFromUTF16(t),
            r = btoa(encodeURIComponent(n));
          await ow.controller.set("state", r),
    ...
    Rk = {
      update: async function (e) {
        ...
        for (const e of t.later.posts) {
          ...
          let t, n;
          try {
            t = await ow.controller.get(`later.post-${e.id}`);
          } catch (e) {
            console.error(e);
            continue;
          }
    ...
    update: async function (e) {
      ...
      for (const e of t.later.posts) {
        ...
        if (t) {
          if (t.blob)
            try {
              n = await zk.controller.save(t.blob, "later");
            } catch (e) {
              console.error(e);
            }
          if (t.previewBlob)
            try {
              r = await zk.controller.save(t.previewBlob, "later");
            } catch (e) {
              console.error(e);
            }
          if (t.coverBlob)
            try {
              o = await zk.controller.save(t.coverBlob, "later");
            } catch (e) {
              console.error(e);
            }
          try {
            await ow.controller.delete(`later.post-${e.id}`);
          } catch (e) {
            console.error(e);
          }
          delete e.previewUrl,
      ...
      for (const e of t.later.posts)
        for (const t of e.mediaList)
          try {
            ...
            const e = await Xk.controller.read(t.fileId);
            if (!e) continue;
  ...
  r(wD, "default", function () {
    ...
    n(wD);
  const xD = {
      "version-130": ix(ax).default,
      "version-131": ix(lx).default,
      "version-132": ix(cx).default,
      "version-133": ix(fx).default,
      "version-134": ix(hx).default,
      "version-135": ix(mx).default,
      "version-136": ix(yx).default,
      "version-137": ix(wx).default,
      "version-138": ix(xx).default,
      "version-139": ix(Px).default,
      "version-140": ix(Dx).default,
      "version-141": ix(Ex).default,
      "version-142": ix(Cx).default,
      "version-143": ix(Ax).default,
      "version-144": ix(Mx).default,
      "version-145": ix(Ux).default,
      "version-146": ix(Bx).default,
      "version-147": ix(Lx).default,
      "version-148": ix($x).default,
      "version-149": ix(Hx).default,
      "version-150": ix(Gx).default,
      "version-151": ix(Jx).default,
      "version-152": ix(Yx).default,
      "version-153": ix(Xx).default,
      "version-154": ix(eS).default,
      "version-155": ix(nS).default,
      "version-156": ix(oS).default,
      "version-157": ix(aS).default,
      "version-158": ix(lS).default,
      "version-159": ix(cS).default,
      "version-160": ix(fS).default,
      "version-161": ix(hS).default,
      "version-162": ix(mS).default,
      "version-163": ix(yS).default,
      "version-164": ix(wS).default,
      "version-165": ix(xS).default,
      "version-166": ix(PS).default,
      "version-167": ix(DS).default,
      "version-168": ix(ES).default,
      "version-169": ix(CS).default,
      "version-170": ix(AS).default,
      "version-171": ix(MS).default,
      "version-172": ix(US).default,
      "version-173": ix(BS).default,
      "version-174": ix(LS).default,
      "version-175": ix($S).default,
      "version-176": ix(HS).default,
      "version-177": ix(GS).default,
      "version-178": ix(JS).default,
      "version-179": ix(YS).default,
      "version-180": ix(XS).default,
      "version-181": ix(eP).default,
      "version-182": ix(nP).default,
      "version-183": ix(oP).default,
      "version-184": ix(aP).default,
      "version-185": ix(lP).default,
      "version-186": ix(cP).default,
      "version-187": ix(fP).default,
      "version-188": ix(hP).default,
      "version-189": ix(mP).default,
      "version-190": ix(yP).default,
      "version-191": ix(wP).default,
      "version-192": ix(xP).default,
      "version-193": ix(PP).default,
      "version-194": ix(DP).default,
      "version-195": ix(EP).default,
      "version-196": ix(CP).default,
      "version-197": ix(AP).default,
      "version-198": ix(MP).default,
      "version-199": ix(UP).default,
      "version-200": ix(BP).default,
      "version-201": ix(LP).default,
      "version-202": ix($P).default,
      "version-203": ix(HP).default,
      "version-204": ix(GP).default,
      "version-205": ix(JP).default,
      "version-206": ix(YP).default,
      "version-207": ix(XP).default,
      "version-208": ix(ek).default,
      "version-209": ix(nk).default,
      "version-210": ix(ok).default,
      "version-211": ix(ak).default,
      "version-212": ix(lk).default,
      "version-213": ix(ck).default,
      "version-214": ix(fk).default,
      "version-215": ix(hk).default,
      "version-216": ix(mk).default,
      "version-217": ix(yk).default,
      "version-218": ix(wk).default,
      "version-219": ix(xk).default,
      "version-220": ix(Pk).default,
      "version-221": ix(Dk).default,
      "version-222": ix(Ek).default,
      "version-223": ix(Ck).default,
      "version-224": ix(Ak).default,
      "version-225": ix(Mk).default,
      "version-226": ix(Uk).default,
      "version-227": ix(Bk).default,
      "version-228": ix(Lk).default,
      "version-229": ix($k).default,
      "version-230": ix(qk).default,
      "version-231": ix(Wk).default,
      "version-232": ix(Qk).default,
      "version-233": ix(Kk).default,
      "version-234": ix(eD).default,
      "version-235": ix(nD).default,
      "version-236": ix(oD).default,
      "version-237": ix(aD).default,
      "version-238": ix(lD).default,
      "version-239": ix(cD).default,
      "version-240": ix(fD).default,
      "version-241": ix(hD).default,
      "version-242": ix(mD).default,
      "version-243": ix(yD).default,
      "version-244": ix(wD).default,
    },
    SD = {
      versioners: {},
      init: function () {
        ...
        Object.keys(xD)
          ...
          .forEach((e) => {
            this.versioners[e.version] = xD[e.key];
          });
      },
      update: async function (e) {
        let t = e,
  ...
  const PD = function (e) {
    ...
    { $influx: kD } = app;
  var DD = kD.action("state.setup-default-state", (e) =>
    e.whatsNew
      ? e
      : {
          version: 244,
  ...
  var ED = {
    controller: {
      init: async function () {
        DD.dispatch(), await this._update();
      },
      _update: async function () {
        let e = ID.model.state;
        (e = await SD.update(e)),
          (e = PD(e)),
          e !== ID.model.state && bb.dispatch(e);
      },
    },
  };
  const { $startup: TD, $chromeBus: CD, $insights: FD, $inspiration: AD } = app;
  (TD.controller = {
    init: async function () {
      let e = !1;
      CD.on("bg.is-ready", () => e),
      ...
      const t = tx.controller.getVersion();
      console.log(`chrome version is: ${t}`),
        ox.init(),
        await X_.controller.init(),
        Qy.controller.init({
          dsn: "https://bea0900834f541bca8157710f7fd31fe@sentry.io/1547551",
        }),
        nx.controller.init(),
        await Fw.controller.init(),
        U_.controller.init(),
        await Eb.controller.init(),
        N_.controller.init(),
        await Jw.controller.init("background", !0),
        await ED.controller.init(),
        Ib.controller.init(),
        t_.controller.init(),
        n_.controller.init(),
        FD.controller.init(),
        ...
        gb.controller.init(),
        AD.controller.init(),
        Qb.controller.init().sendPageview().sendInstall(),
        Tb.controller.init(),
        await Tb.controller.updateUser(),
        Zg.controller.init(),
